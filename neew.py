...
class GanttChartApp:
    def __init__(self, root):
        ...
        # --- НОВОЕ: Создаем фрейм для холста и скроллбаров ---
        canvas_frame = ttk.Frame(self.main_frame)
        canvas_frame.pack(fill=tk.BOTH, expand=True)

        # --- НОВОЕ: Создаем и настраиваем скроллбары ---
        ...
        # --- НОВОЕ: Размещаем скроллбары и холст с помощью grid для лучшего контроля ---
    ...
    def _zoom(self, factor, x, y):
        ...
        # Эта строка теперь обновляет и область для панорамирования, и диапазон для скроллбаров
    ...
    def draw_gantt(self, data):
        ...
        # Эта строка теперь инициализирует и область для панорамирования, и диапазон для скроллбаров
        bbox = self.canvas.bbox("all")
        if bbox:
            self.canvas.config(scrollregion=bbox)
